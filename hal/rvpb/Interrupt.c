...
extern volatile GicCput_t *const GicCpu;
extern volatile GicDist_t *const GicDist;
...
void Hal_interrupt_enable(uint32_t interrupt_num)
{
    ...
    {
        SET_BIT(GicDist->setenable2, bit_num - 32U);
    }
}
...
void Hal_interrupt_disable(uint32_t interrupt_num)
{
    ...
    if(bit_num < 32U)
    {
        GicDist->clearenable1 = 1U << bit_num;
    }
    else
    {
        GicDist->clearenable2 = 1U << (bit_num - 32U);
    }
}
...
void Hal_interrupt_run_handler(void)
{
    ...
    if(interrupt_num < INTERRUPT_HANDLER_NUM && sHandlers[interrupt_num] != NULL)
    {
